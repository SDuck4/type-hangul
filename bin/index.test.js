...
describe("type()", () => {
    describe("text 옵션으로 출력할 텍스트를 설정할 수 있다.", () => {
        it("text가 설정되어 있으면 기존 텍스트는 무시하고 타이핑된다.", () => {
            ...
            const selector = "#target", options = {
                text: "타이핑",
                intervalType: 1000
            }, target = document.querySelector("#target"), textProcess = ["ㅌ", "타", "탕", "타이", "타잎", "타이피", "타이핑"];
            _1.default.type(selector, options);
    ...
    describe("append 옵션으로 기존 텍스트 뒤에 출력할 수 있다.", () => {
        it("append가 true일 때 기존 텍스트 뒤에 출력된다.", () => {
            ...
            const selector = "#target", options = {
                ...
                append: true,
                intervalType: 1000
            }, target = document.querySelector("#target"), textProcess = [
                "이젠ㅇ",
                "이젠아",
                "이젠안",
                "이젠안ㄴ",
    ...
    describe("selector로 여러 DOM을 선택해 적용할 수 있다.", () => {
        it("여러 DOM이 선택될 때, 선택된 모든 DOM에 타이핑 효과가 적용된다.", () => {
            ...
            const selector = ".target", options = {
                intervalType: 1000
            }, t1 = document.querySelector("#t1"), t1Process = ["ㅎ", "하", "한", "한ㄱ", "한그", "한글"], t2 = document.querySelector("#t2"), t2Process = ["ㅌ", "타", "탕", "타이", "타잎", "타이피", "타이핑"], t3 = document.querySelector("#t3"), t3Process = ["ㅎ", "효", "횩", "효고", "효과"];
            _1.default.type(selector, options);
